...
5. **Object Storage (MinIO)**: Stores processed media files with automatic 30-day expiry after ML processing completes.
6. **Structured Logging**: JSON-formatted logs across all modules written to stdout and `logs/app.log` for audit and replay.
...
# 3️⃣ Start only the API service (brings DB and MinIO up with it)
...
| ML running   | `processing`  | ML stub invoked                          |
...
- **UI:** http://localhost:9001 — login with `minioadmin / minioadmin`
